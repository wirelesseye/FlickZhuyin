...
FlickZhuyin 所附的 `flickzhuyin.sqlite3` 是上述詞典資料的格式轉換版本，依 CC BY-SA 3.0 提供。修改內容包括拼音至注音轉換、資料去重、欄位正規化、索引建立及 SQLite 封裝。來源、commit、SHA-256、詞典版本及 compiler 版本均保存在 repository 的 `Vendor/rime-terra-pinyin/SOURCE.json` 與 SQLite `metadata` table。
